...
public abstract class AbstractSQLConfig implements SQLConfig {
        ...
        if (ckeys != null && ckeys.length > 0) {
            ...
            for (int i = 0; i < ckeys.length; i++) {
                ...
                if (ckeys[i].contains("'")) {
                    ...
                    // 1.字符串不是字段也没有别名,所以不解析别名 2. 是字符串，进行预编译，使用getValue()
     ...
     * @return <p>use entrySet+getValue() to replace keySet+get() to enhance efficiency</p>
     */
    @JSONField(serialize = false)
    @Override
     ...
     * @return
     * @throws Exception <p>use entrySet+getValue() to replace keySet+get() to enhance efficiency</p>
     */
    @JSONField(serialize = false)
